...
class OAuth(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        token = data['token']
        error_codes = {
            'access_denied': status.HTTP_401_UNAUTHORIZED
        }
        ...
        return Response(
            {
                ...
                'created': (timezone.localize(datetime.utcnow()) - user.date_joined ) < timedelta(seconds=10)
            },
            status=status.HTTP_201_CREATED if (timezone.localize(datetime.utcnow()) - user.date_joined ) < timedelta(seconds=10) else status.HTTP_200_OK
        )
