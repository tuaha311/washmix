...
from ..modules.oauth import get_user_from_token, get_user_by_email
...
class SocialAppLoginRefreshTokenView(TokenView):
    def post(self, request, *args, **kwargs):
        ...
        if 'error' in response:
            ...
            if not social_user:
                raise AuthenticationFailed(response.get('error_description', '') or response.get('error', ''))
        else:
        ...
        return Response(
            {
                'token': '',
                'refresh_token': '',
                'user': UserDataSerializer(instance=social_user, stripes_helper=StripeHelper()).data,
