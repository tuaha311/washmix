...
class UsersTest(APITestCase):
    ...
    def test_add_users(self):
        ...
        """
        client = APIClient()
        response = client.post('/users/', {'users': [{'first_name': 'test_first', 'last_name': 'test_last',
                                                      'password': 'test321', 'email': 'test@test.com', 'phone': '00000000'},
                                                     {'first_name': 'test2_first', 'last_name': 'test2_last',
                                                      'email': 'test2@test2.com', 'password': 'test2321', 'phone': '00000000'}]},
                               format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_users_params(self):
        ...
        """
        client = APIClient()
        response = client.post('/users/', {'users': [{'last_name': 'test_last',
                                                      'email': '321@test.com', 'phone': '00000000'}]}, format='json')
    ...
    @patch('app.modules.helpers.StripeHelper.add_customer', side_effect=mock_add_customer)
    @patch('app.modules.helpers.StripeHelper.get_customer', side_effect=mock_get_customer)
    @patch('app.modules.helpers.StripeHelper.get_token', side_effect=mock_get_token)
    @patch('app.modules.helpers.StripeHelper.get_card', side_effect=mock_get_card)
    @patch('middleware.MessageClient.send_message', side_effect=mocked_twilio_create)
    def test_user_purchase(self, *args):
        ...
        response = client.post('/user_purchase/add_card/',
