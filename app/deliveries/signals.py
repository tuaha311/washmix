...
def on_delivery_notify_signal(
    ...
    is_admin = bool(delivery.request.generated_by_admin) #Requests which are generated by admin to charge client
    is_in_store_droppoff = delivery.status == DeliveryStatus.IN_STORE_DROPOFF
    is_in_store_pickup = delivery.status == DeliveryStatus.IN_STORE_PICKUP
    is_instore = delivery.in_store

    if update_fields:
        is_date_updated = "date" in update_fields

    if is_pickup and (is_created or is_date_updated) and not is_admin:
    ...
    if is_dropoff and is_completed and not is_admin:
        # we are adding some delay to wait for database
        # transaction commit
        send_sms.send_with_options(
            kwargs={
    ...
    if is_admin and is_dropoff and is_completed and is_instore:
        send_sms.send_with_options(
            kwargs={
    ...
    if is_admin and is_in_store_pickup and is_instore:
        send_sms.send_with_options(
            kwargs={
                "event": settings.IN_STORE_PICKUP,
                "recipient_list": [number],
                "extra_context": {
                    "client_id": client.id,
                    "delivery_id": delivery.id,
    ...
    if is_pickup and is_completed and is_admin and is_instore:
        # we are adding some delay to wait for database
        # transaction commit
        send_sms.send_with_options(
            kwargs={
