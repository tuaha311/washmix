...
from ..utilities.wm_message_config import (
    order_pickup,
    order_dropoff
)
...
class Order(models.Model):
    ...
    def schedule_notification(self, message_formatter, reminder_time):
        ...
        # Schedule the task
        from app.tasks import send_sms_reminder
        ...
        result = send_sms_reminder.send_with_options(args=(self.pk,
                                                           message_formatter(), settings.TEAM_WASHMIX,),
                                                     delay=reminder_time)
    ...
    def save(self, force_insert=False, force_update=False, using=None,
        ...
        self.task_id_pickup = self.schedule_notification(
            order_pickup(user=self.user, order=self),
            self.pick_up_from_datetime
        )
        self.task_id_dropoff = self.schedule_notification(
            order_dropoff(user=self.user, order=self),
            self.drop_off_from_datetime
        )
...
class Profile(models.Model):
    ...
    # Preferences
    detergents = models.CharField(max_length=50, choices=[(item, item.value) for item in Detergents], null=True)
    starch = models.CharField(max_length=50, choices=[(item, item.value) for item in Starch], null=True)
    no_crease = models.CharField(max_length=50, choices=[(item, item.value) for item in Crease], null=True)
    app_users = models.CharField(max_length=30, choices=[(item, item.value) for item in AppUsers], null=True,
                                 default=AppUsers.REGULAR_USERS.value)
    authentication_provider = models.CharField(max_length=30, choices=[(item, item.value) for item in SignUp], null=True,
...
class Coupons(models.Model):
    ...
    coupon_type = models.CharField(max_length=30, choices=[(item, item.value) for item in CouponType], null=True,
...
class Product(models.Model):
    ...
    product = models.ForeignKey('self', null=True, related_name='children', on_delete=models.CASCADE)
