...
class SendSMSAdmin(ClientAdmin):
    ...
    def get_params(self, request):
        template_param = request.POST.get("template")
        ...
        if template_param:
            template_param = SMSTemplate.objects.get(pk=template_param)
        ...
        if filters:
            ...
            if selected_customers:
                query |= Q(id__in=selected_customers_ids)
            queryset = queryset.filter(query)

        data = {
            "queryset": queryset,
            "selected_template": template_param,
